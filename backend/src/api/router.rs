...
/// F-10：目录缓存 TTL（5 分钟）
/// 目录缓存 TTL (默认 24h, env READER_TOC_CACHE_TTL_MS 覆盖, 单位 ms).
/// 链表分页目录只能串行抓 (下一页 URL 来自当前页解析), 首抓必然慢;
/// 长 TTL + 手动刷新 (refresh=1) + 书架更新 job 是追更感知兜底.
fn toc_cache_ttl_ms() -> i64 {
    static TTL: std::sync::OnceLock<i64> = std::sync::OnceLock::new();
    *TTL.get_or_init(|| {
        std::env::var("READER_TOC_CACHE_TTL_MS")
            .ok()
...
/// simple=1 → 轻量变体: bookSourceGroup/Name/Url + enabled + bookSourceType, 不含规则字段
...
async fn search_book_multi(
    ...
    let include_dead = params.get("all").map(|v| v == "1").unwrap_or(false);
    if !include_dead && single_source_url.is_empty() {
        let now = chrono::Utc::now().timestamp_millis();
        if let Ok(stats) = state.storage.get_source_stats(&namespace).await {
            let dead: std::collections::HashSet<String> = stats
                .iter()
                .filter(|st| {
                    st.confidence() <= 0.05
                        && st.attempts >= 5
                        && now - st.last_failure_at <= 6 * 3_600_000
                })
                .map(|st| st.source_url.clone())
                .collect();
            if !dead.is_empty() {
                sources.retain(|s| !dead.contains(&s.book_source_url));
            }
        }
    }
    if sources.is_empty() {
        ...
        handles.push(tokio::spawn(async move {
            ...
            let (ok, intro_hits, intro_count) = match &res {
                Ok(books) => (
                    ...
                    books
                        .iter()
                        .filter(|b| b.intro.as_deref().is_some_and(|t| !t.trim().is_empty()))
                        .count() as i64,
                    books.len() as i64,
                ),
                Err(_) => (false, 0, 0),
            };
            let surl = source.book_source_url.clone();
            let st = storage.clone();
            let nsx = ns.clone();
            tokio::spawn(async move {
                let _ = st
                    .record_search_outcome(&nsx, &surl, ok, lat, intro_hits, intro_count)
                    .await;
            });
...
/// bookSource 缺省时按书籍/章节 URL 域名回算书源: 历史缺陷是空参 LIKE '%' 任意取
/// 第一条源, 跨站误解析 (导航 li 混进目录等); 无同域源时回退旧空参语义.
async fn resolve_book_source_for_url(
    state: &AppState,
    ns: &str,
    param: &str,
    book_url: &str,
    ...
    if !param.trim().is_empty() {
        return resolve_book_source(state, ns, param).await;
    }
    let host = url::Url::parse(book_url).ok()?.host_str()?.to_string();
    if let Ok(sources) = state.storage.get_book_sources(ns).await {
        if let Some(hit) = sources.into_iter().find(|s| {
            url::Url::parse(&s.book_source_url)
                .ok()
    ...
    resolve_book_source(state, ns, param).await
}
...
async fn set_book_source(
        ...
        .await
    {
        ...
        Ok(_) => {
            // 换源后旧 URL 详情缓存失效, 新 URL 详情落缓存
            let _ = state.storage.clear_book_info_cache(&namespace, &book_url).await;
            let info_value = serde_json::to_value(info.clone()).unwrap_or(serde_json::Value::Null);
            let _ = state
                .storage
                .set_book_info_cache(&namespace, &new_url, &info_value)
                .await;
...
async fn get_book_info(
    ...
    // DB 持久详情缓存 (6h TTL): 书架书与跨重启都秒回; 命中后前端仍按 staleTime 静默刷新
    if let Ok(Some(cached)) = state
        .storage
        .get_book_info_cache(&namespace, &url, 6 * 3600 * 1000)
        .await
    {
        return Json(ReturnData::ok(cached));
    }
    ...
    let Some(source) = resolve_book_source_for_url(&state, &namespace, &bs_param, &url).await else {
        return Json(ReturnData::err("书源不存在"));
    };
    ...
    .await
    {
        Ok(mut info) => {
            ...
            // DB 持久详情缓存: 所有书(含书架书)落盘, 6h TTL 内秒回
            let info_value = serde_json::to_value(info).unwrap_or(serde_json::Value::Null);
            let _ = state
                .storage
                .set_book_info_cache(&namespace, &url, &info_value)
                .await;
            Json(ReturnData::ok(info_value))
        }
        Err(e) => {
...
async fn get_book_toc(
    ...
    let refresh = param_of(&params, body_json.as_ref(), "refresh")
        .parse::<i64>()
        .unwrap_or(0);
    // 目录缓存命中立即返回: 跳过下方「抓书页推导 tocUrl」——该抓取每请求 2-3s,
    // 缓存命中时纯属浪费 (性能实测: getChapterList 命中路径 2-4s 的全部来源)
    if refresh <= 0 {
        if let Ok(Some(cached)) = state
            .storage
            ...
            .await
        {
            ...
            {
                tracing::debug!("getBookToc 前置命中目录缓存 [{toc_url}]");
                return Json(ReturnData::ok(
    ...
    // 未入架且无 tocUrl 参数: 部分源目录在独立页 (啃书 /xiaoshuo/784/0/),
    // 需先按 ruleBookInfo.tocUrl 从书页解析; 否则把书页当目录页抓 → 0 章
    if !url_param.is_empty() && toc_url == url_param {
        let bs_early = param_of(&params, body_json.as_ref(), "bookSource");
        if let Some(source) =
    ...
    // F-10：目录缓存命中（同 tocUrl 直读）直接返回，不依赖书源；
    // 推导过 tocUrl 的 miss 路径在此二次查缓存
    if refresh <= 0 {
        if let Ok(Some(cached)) = state
            .storage
            .get_toc_cache(&namespace, &toc_url, toc_cache_ttl_ms())
    ...
    let Some(source) = resolve_book_source_for_url(&state, &namespace, &bs_param, &toc_url).await else {
...
async fn get_book_content(
    ...
    let bs_param = param_of(&params, body_json.as_ref(), "bookSource");
    let Some(source) = resolve_book_source_for_url(&state, &namespace, &bs_param, &chapter_url).await else {
        return Json(ReturnData::err("书源不存在"));
    };
    ...
    .await
    {
        ...
        Err(e) => {
            ...
            let _ = state
                .storage
                .record_path_outcome(&namespace, &source.book_source_url, "content", false)
                .await;
...
/// GET/POST /reader3/getSourceStats：书源置信度统计 (搜索排序依据/清理无效源 UI)
async fn get_source_stats_handler(
    State(state): State<AppState>,
    Query(params): Query<HashMap<String, String>>,
    headers: HeaderMap,
) -> Json<ReturnData> {
    ...
    let stats = state.storage.get_source_stats(&namespace).await.unwrap_or_default();
    let list: Vec<serde_json::Value> = stats
        .iter()
        .map(|st| {
            serde_json::json!({
                "sourceUrl": st.source_url,
                "confidence": (st.confidence() * 100.0).round() / 100.0,
                "attempts": st.attempts,
                "successRate": (st.success_rate() * 100.0).round() / 100.0,
                "avgLatencyMs": st.avg_latency_ms(),
                "consecFailures": st.consec_failures,
                "lastFailureAt": st.last_failure_at,
            })
        })
        .collect();
                ...
                let (ok, intro_hits, intro_count) = match &res {
                    Ok(books) => (
                        ...
                        books
                            .iter()
                            .filter(|b| {
                                b.intro.as_deref().is_some_and(|t| !t.trim().is_empty())
                            })
                            .count() as i64,
                        books.len() as i64,
                    ),
...
/// SSE 并发数生效值：缺省 24（实测家庭出口 50 并发壅塞致首事件 30s，20-24 并发流畅 1s 内出事件），
...
async fn delete_book_cache(
    ...
    // 详情缓存一并失效 (避免删缓存后详情仍旧)
    let _ = state.storage.clear_book_info_cache(&namespace, &url).await;
